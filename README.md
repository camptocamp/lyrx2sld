...
## Docker composition (test results and debug)
...
The SLD file will be saved to the same folder and sent to GeoServer. In the GeoServer GUI, you'll then need to publish the 
layer and to link it to the new style. The style will be named `Defaut Styler`.
...
Finally, stop the composition with:
```
make stop
```